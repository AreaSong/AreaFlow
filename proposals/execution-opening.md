...
> Status: Proposed. 当前实现只包含文档中明确列出的 dry-run、read-only 和受控写入类型。
...
v0.6 的每条能力必须保留
[`v0.6-worker-beta-contract.md`](../docs/history/v1.0/contracts/v0.6-worker-beta-contract.md) 定义的 scope label 或等价 safety facts。
...
[`high-risk-apply-ladder.md`](./high-risk-apply.md) 的状态词、apply packet、suspension rule 和
