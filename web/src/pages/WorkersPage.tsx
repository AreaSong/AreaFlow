...
export function WorkersPage() {
  ...
  const data = useAsyncValue(async () => {
    const [workerCollection, routeWorkerCollection, pool, schedule] = await Promise.all([
    ...
    return { workers: workerCollection.workers.map((item) => item.worker), routeWorker: routeWorkerCollection?.workers[0]?.worker, pool, schedule };
  ...
  const sortedWorkers = useMemo(() => [...(data.data?.workers ?? [])].sort((left, right) => {
  ...
  const worker = selectedWorker
    ? data.data?.workers.find((item) => item.worker_key === selectedWorker) ?? data.data?.routeWorker
    : data.data?.workers[0];
  ...
  return <div className="page"><PageHeader eyebrow="Execution capacity" title="Workers" description="Worker registration, heartbeat health, capabilities, pool capacity, and scheduling decisions." />
    {data.loading ? <LoadingState label="Loading workers" /> : data.error ? <ErrorState message={data.error} onRetry={data.retry} /> : data.data ? <>
      <div className="summary-grid"><Metric label="Workers" value={data.data.pool.total_workers} /><Metric label="Online" value={data.data.pool.total_online_workers} /><Metric label="Active leases" value={data.data.pool.total_active_leases} /><Metric label="Queued tasks" value={data.data.pool.total_queued_tasks} /></div>
      <div className="resource-layout"><Section title="Worker registry" className="resource-index"><ListControls query={workerList.query} onQueryChange={workerList.setQuery} page={workerList.page} pageCount={workerList.pageCount} total={workerList.total} onPageChange={workerList.setPage} placeholder="Search workers" sortValue={sort} onSortChange={setSort} sortOptions={[{ value: "recent", label: "Recently updated" }, { value: "key", label: "Worker key A-Z" }, { value: "status", label: "Status A-Z" }]} /><div className="resource-list dense">{workerList.items.length ? workerList.items.map((item) => <button key={item.worker_key} className={item.worker_key === worker?.worker_key ? "active" : ""} onClick={() => { setSelectedWorker(item.worker_key); navigate(`/workers/${encodeURIComponent(item.worker_key)}?project=${encodeURIComponent(selectedProjectKey)}`); }}><div><strong>{item.worker_key}</strong><small>{item.worker_type} / {item.hostname}</small></div><StatusBadge value={item.status} /></button>) : <EmptyState message={data.data.workers.length ? "No workers match this search" : "No workers registered"} />}</div></Section>
      ...
      <Section title="Schedule preview" description={`${data.data.schedule.policy.strategy}; dry run: ${data.data.schedule.policy.dry_run_only}`}><div className="table-list">{data.data.schedule.projects.map((project) => <div key={project.project.key}><div><strong>{project.project.name || project.project.key}</strong><small>{project.next_action}; {project.available_slots} slots</small></div><StatusBadge value={project.recommended ? "ready" : "blocked"} /></div>)}</div></Section>
    </> : null}
  </div>;
}
