...
if [[ -z "${AREAFLOW_DATABASE_URL:-}" ]]; then
  echo "smoke-completion-audit-release-candidate-snapshot: skipped; AREAFLOW_DATABASE_URL is not set"
  exit 0
fi

project_key="areamatrix"
fixture_dir="$(mktemp -d "${TMPDIR:-/tmp}/areaflow-completion-audit-rc.XXXXXX")"
...
case "${fixture_dir}" in
  ...
  *)
    echo "smoke-completion-audit-release-candidate-snapshot: refusing unsafe fixture dir: ${fixture_dir}" >&2
    exit 1
    ;;
esac
...
cleanup() {
  if [[ "${AREAFLOW_KEEP_FIXTURE:-0}" == "1" ]]; then
    echo "smoke-completion-audit-release-candidate-snapshot: keeping fixture at ${fixture_dir}"
    return
  fi
  rm -rf "${fixture_dir}"
}
...
assert_contains() {
  ...
  if ! grep -Fq -- "${pattern}" <<<"${output}"; then
    echo "smoke-completion-audit-release-candidate-snapshot: expected output to contain pattern: ${pattern}" >&2
    exit 1
  fi
}
...
assert_line() {
  ...
  if ! grep -Fxq -- "${pattern}" <<<"${output}"; then
    echo "smoke-completion-audit-release-candidate-snapshot: expected output line: ${pattern}" >&2
    exit 1
  fi
}
...
assert_not_contains() {
  ...
  if grep -Fq -- "${pattern}" <<<"${output}"; then
    echo "smoke-completion-audit-release-candidate-snapshot: expected output to omit pattern: ${pattern}" >&2
    exit 1
  fi
}
...
if hits:
    print(
        "smoke-completion-audit-release-candidate-snapshot: completion items unexpectedly blocked by "
        + blocker
        + ": "
        + ", ".join(hits),
        file=sys.stderr,
...
assert_positive_value() {
  ...
  if ! [[ "${value}" =~ ^[1-9][0-9]*$ ]]; then
    echo "smoke-completion-audit-release-candidate-snapshot: expected positive ${label}, got ${value}" >&2
    exit 1
  fi
}
...
assert_sha256_value() {
  ...
  if ! [[ "${value}" =~ ^[0-9a-f]{64}$ ]]; then
    echo "smoke-completion-audit-release-candidate-snapshot: expected sha256 ${label}, got ${value}" >&2
    exit 1
  fi
}
...
if [[ "${check_real_areamatrix}" == "1" ]]; then
  ...
  echo "smoke-completion-audit-release-candidate-snapshot: skipping real AreaMatrix fingerprint check; set AREAFLOW_SMOKE_CHECK_REAL_AREAMATRIX=1 to enable"
fi
...
# Release Candidate Snapshot Project Docs
EOF

cat >"${project_root}/workflow/README.md" <<'EOF'
# Release Candidate Snapshot Workflow
EOF

cat >"${project_root}/workflow/residuals/residuals.yaml" <<'EOF'
items:
  - id: global-rc-fixture-note
    status: reference-only
    type: fixture
    title: Release candidate snapshot fixture residual
...
version_residuals:
  - version: v1-mvp
    ...
    summary: Release candidate snapshot fixture version residuals
EOF

cat >"${project_root}/workflow/templates/README.md" <<'EOF'
# Release Candidate Snapshot Templates
EOF
...
version_status:
  ...
  fixture: true
items:
  - id: v1-rc-fixture-residual
    status: reference-only
    type: fixture
    title: Release candidate snapshot v1 fixture residual
    source: workflow/versions/v1-mvp/residuals/residuals.yaml
...
# Release Candidate Snapshot Fixture Template Version
EOF

cat >"${project_root}/tasks/indexes/residuals.md" <<'EOF'
# Release Candidate Snapshot Fixture Residual Index
EOF

cat >"${project_root}/tasks/backlog/README.md" <<'EOF'
# Release Candidate Snapshot Fixture Backlog
EOF

local_release_artifact="${artifact_root}/completion-audit-release-candidate.json"
cat >"${local_release_artifact}" <<'EOF'
{"kind":"completion_audit_release_candidate_fixture","release_readiness_local_artifact":true}
EOF
...
project:
  id: ${project_key}
  name: AreaMatrix Release Candidate Snapshot Project
  root: ${project_root}
  kind: product-repo
  adapter: areamatrix
...
validation_scope="fixture_release_candidate_snapshot_review"
archive_fact_args=()
...
echo "smoke-completion-audit-release-candidate-snapshot: migrate up"
go run ./cmd/areaflow migrate up

echo "smoke-completion-audit-release-candidate-snapshot: project add ${config_path}"
go run ./cmd/areaflow project add --config "${config_path}"

echo "smoke-completion-audit-release-candidate-snapshot: project import ${project_key}"
...
echo "smoke-completion-audit-release-candidate-snapshot: localize imported fixture artifacts for release readiness"
...
echo "smoke-completion-audit-release-candidate-snapshot: project status-projection-authorization ${project_key}"
...
if [[ -f "${fixture_status}" ]]; then
  echo "smoke-completion-audit-release-candidate-snapshot: authorization preview unexpectedly wrote ${fixture_status}" >&2
  exit 1
fi
...
echo "smoke-completion-audit-release-candidate-snapshot: project status-projection-apply-packet ${project_key}"
apply_packet_ready_json="$(
  go run ./cmd/areaflow project status-projection-apply-packet "${project_key}" --json \
    --explicit-approval \
    --approval-actor "smoke-completion-audit-release-candidate-snapshot" \
...
assert_contains "${apply_packet_ready_json}" '"approval_actor": "smoke-completion-audit-release-candidate-snapshot"'
...
if [[ -f "${fixture_status}" ]]; then
  echo "smoke-completion-audit-release-candidate-snapshot: apply packet preview unexpectedly wrote ${fixture_status}" >&2
  exit 1
fi
protected_path_fingerprint_sha256="$(json_get "${apply_packet_ready_json}" "packet.protected_path_fingerprint_sha256")"

echo "smoke-completion-audit-release-candidate-snapshot: project status-projection-apply-gate ${project_key}"
...
if [[ -f "${fixture_status}" ]]; then
  echo "smoke-completion-audit-release-candidate-snapshot: apply gate unexpectedly wrote ${fixture_status}" >&2
  exit 1
fi

echo "smoke-completion-audit-release-candidate-snapshot: project status-projection-apply ${project_key}"
...
if [[ ! -f "${fixture_status}" ]]; then
  echo "smoke-completion-audit-release-candidate-snapshot: expected fixture status export at ${fixture_status}" >&2
  exit 1
fi

echo "smoke-completion-audit-release-candidate-snapshot: validate status projection schema"
python3 scripts/validate-status-projection-schema.py schemas/status-projection.schema.json "${fixture_status}"

echo "smoke-completion-audit-release-candidate-snapshot: project status-projections --json"
...
echo "smoke-completion-audit-release-candidate-snapshot: seed release final gate fixture inputs"
...
artifact_seed AS (
  ...
  SELECT
    id,
    'release_candidate_snapshot_fixture',
    'local',
    :'artifact_uri',
    'completion-audit-release-candidate.json',
    :'artifact_sha',
    :'artifact_size'::bigint,
    'application/json',
    '{"fixture_only":true,"purpose":"release_candidate_snapshot_readiness"}'::jsonb
  FROM project_scope
  RETURNING id
),
...
echo "smoke-completion-audit-release-candidate-snapshot: collect E6 output binding"
...
archive_preview_json="$(go run ./cmd/areaflow artifact archive-preview "${project_key}" \
  --idempotency-key "completion-audit-rc:archive-preview:${project_key}" \
  --reason "collect release candidate backup restore binding" \
  --json)"
...
recorded_json="$(go run ./cmd/areaflow completion source-alignment-proof record "${project_key}" \
  ...
  --evidence-uri "${evidence_root}#source-alignment-reviewed" \
  --idempotency-key "completion-audit-rc:source-alignment:${project_key}" \
  --reason "record reviewed source alignment evidence" \
  --json)"
...
recorded_json="$(go run ./cmd/areaflow completion task-matrix-proof record "${project_key}" \
  ...
  --evidence-uri "${evidence_root}#task-matrix-reviewed" \
  --idempotency-key "completion-audit-rc:task-matrix:${project_key}" \
  --reason "record reviewed task matrix evidence" \
  --json)"
assert_contains "${recorded_json}" '"proof_status": "complete"'
...
recorded_json="$(go run ./cmd/areaflow completion validation-proof record "${project_key}" \
  ...
  --summary "release candidate validation reviewed" \
  --evidence-uri "${evidence_root}#validation-reviewed" \
  "${validation_command_args[@]}" \
  ...
  --idempotency-key "completion-audit-rc:validation:${project_key}" \
  --reason "record reviewed validation evidence" \
  --json)"
assert_contains "${recorded_json}" '"proof_status": "complete"'
...
archive_recorded_json="$(go run ./cmd/areaflow completion archive-proof record "${project_key}" \
  ...
  "${archive_binding_args[@]}" \
  --summary "release candidate archive reviewed" \
  --evidence-uri "${evidence_root}#archive-reviewed" \
  "${review_flags[@]}" \
  --idempotency-key "completion-audit-rc:archive:${project_key}" \
  --reason "record reviewed archive evidence" \
  --json)"
...
shim_recorded_json="$(go run ./cmd/areaflow completion shim-retirement-proof record "${project_key}" \
  ...
  "${shim_binding_args[@]}" \
  --summary "release candidate shim retirement reviewed" \
  --evidence-uri "${evidence_root}#shim-retirement-reviewed" \
  "${review_flags[@]}" \
  --idempotency-key "completion-audit-rc:shim-retirement:${project_key}" \
  --reason "record reviewed shim retirement evidence" \
  --json)"
...
recorded_json="$(go run ./cmd/areaflow completion execution-cutover-proof record "${project_key}" \
  ...
  "${execution_cutover_binding_args[@]}" \
  --summary "release candidate execution cutover reviewed" \
  --evidence-uri "${evidence_root}#execution-cutover-reviewed" \
  "${review_flags[@]}" \
  --idempotency-key "completion-audit-rc:execution-cutover:${project_key}" \
  --reason "record reviewed execution cutover evidence" \
  --json)"
assert_contains "${recorded_json}" '"proof_status": "complete"'
...
recorded_json="$(go run ./cmd/areaflow completion release-packaging-proof record "${project_key}" \
  ...
  --summary "release candidate packaging reviewed" \
  --evidence-uri "${evidence_root}#release-packaging-reviewed" \
  --idempotency-key "completion-audit-rc:release-packaging:${project_key}" \
  --reason "record reviewed release packaging evidence" \
  --json)"
assert_contains "${recorded_json}" '"proof_status": "complete"'
...
recorded_json="$(go run ./cmd/areaflow completion backup-restore-proof record "${project_key}" \
  ...
  --summary "release candidate backup restore reviewed" \
  --evidence-uri "${evidence_root}#backup-restore-reviewed" \
  "${backup_restore_binding_args[@]}" \
  --idempotency-key "completion-audit-rc:backup-restore:${project_key}" \
  --reason "record reviewed backup restore evidence" \
  --json)"
assert_contains "${recorded_json}" '"proof_status": "complete"'
...
recorded_json="$(go run ./cmd/areaflow completion security-closure-proof record "${project_key}" \
  ...
  --summary "release candidate security closure reviewed" \
  --evidence-uri "${evidence_root}#security-closure-reviewed" \
  --idempotency-key "completion-audit-rc:security-closure:${project_key}" \
  --reason "record reviewed security closure evidence" \
  --json)"
assert_contains "${recorded_json}" '"proof_status": "complete"'
...
recorded_json="$(go run ./cmd/areaflow ops smoke-proof record "${project_key}" \
  --key manual_ops_smoke_review \
  --status pass \
  --summary "release candidate operations reviewed" \
  --evidence-uri "${evidence_root}#operations-reviewed" \
  --idempotency-key "completion-audit-rc:operations:${project_key}" \
  --reason "record reviewed operations evidence" \
  --json)"
...
recorded_json="$(go run ./cmd/areaflow completion protected-path-proof record "${project_key}" \
  --status clean \
  --summary "release candidate protected path review was clean" \
  --evidence-uri "${evidence_root}#protected-path-reviewed" \
  --idempotency-key "completion-audit-rc:protected-path:${project_key}" \
  --reason "record reviewed protected path evidence" \
  --json)"
...
echo "smoke-completion-audit-release-candidate-snapshot: completion audit should stay blocked by fixture AreaMatrix identity"
...
assert_not_contains "${completion_json}" '"scripts/'
assert_not_contains "${completion_json}" '"local:'
assert_not_contains "${completion_json}" '"fixture:'

echo "smoke-completion-audit-release-candidate-snapshot: release candidate snapshot should fail closed before fixture identity can be sealed"
set +e
snapshot_error="$(go run ./cmd/areaflow completion audit-snapshot record "${project_key}" \
  --release-candidate "v1.0-rc1" \
  --evidence-class "release_candidate" \
  --summary "reviewed release candidate completion audit evidence bundle" \
  --evidence-uri "${evidence_root}#release-candidate-reviewed" \
  --review-decision "approved" \
  --reviewed-by "release-owner" \
  --reviewed-at "2026-07-04T12:00:00Z" \
  --idempotency-key "completion-audit-rc:snapshot:${project_key}" \
  --reason "record reviewed release candidate snapshot" \
  --json 2>&1)"
snapshot_rc=$?
set -e
if [[ ${snapshot_rc} -eq 0 ]]; then
  echo "smoke-completion-audit-release-candidate-snapshot: expected fixture release_candidate snapshot record to fail" >&2
  exit 1
fi
assert_contains "${snapshot_error}" "completion audit snapshot requires current audit status complete"
assert_not_contains "${snapshot_error}" '"status": "recorded"'
assert_not_contains "${snapshot_error}" '"release_candidate_snapshot": true'
...
assert_contains "${snapshot_readiness_json}" '"real_100_status": "blocked"'
assert_contains "${snapshot_readiness_json}" '"readiness_scope": "completion_audit_evidence_only"'
assert_contains "${snapshot_readiness_json}" '"claim_scope": "completion_audit_evidence_only"'
assert_contains "${snapshot_readiness_json}" '"not_real_100": true'
assert_contains "${snapshot_readiness_json}" '"evidence_only": true'
assert_contains "${snapshot_readiness_json}" '"status_alone_is_not_completion": true'
assert_contains "${snapshot_readiness_json}" '"release_candidate_decision": "requires_release_candidate_snapshot"'
assert_contains "${snapshot_readiness_json}" '"real_100_blockers": ['
assert_contains "${snapshot_readiness_json}" '"package_a_status_projection_apply_provenance_missing"'
assert_contains "${snapshot_readiness_json}" '"release_candidate_snapshot_not_ready"'
...
assert_not_contains "${snapshot_readiness_json}" '"status": "ready"'
assert_not_contains "${snapshot_readiness_json}" '"completion_audit_snapshot_release_candidate_present"'
assert_not_contains "${snapshot_readiness_json}" '"completion_audit_snapshot_fixture_only"'
assert_not_contains "${snapshot_readiness_json}" '"completion_audit_snapshot_mechanism_evidence_uri"'
assert_not_contains "${snapshot_readiness_json}" '"completion_audit_snapshot_mechanism_proof_evidence_uri"'
assert_not_contains "${snapshot_readiness_json}" '"completion_audit_snapshot_unsafe_side_effects"'
...
if [[ "${check_real_areamatrix}" == "1" ]]; then
  ...
  if [[ "${real_status_before}" != "${real_status_after}" ]]; then
    echo "smoke-completion-audit-release-candidate-snapshot: real AreaMatrix status changed unexpectedly: ${real_areamatrix_status}" >&2
    exit 1
  fi

  if [[ "${real_readme_before}" != "${real_readme_after}" ]]; then
    echo "smoke-completion-audit-release-candidate-snapshot: real AreaMatrix workflow README changed unexpectedly: ${real_areamatrix_readme}" >&2
    exit 1
  fi
fi

echo "smoke-completion-audit-release-candidate-snapshot: pass fail-closed fixture=${fixture_dir}"
