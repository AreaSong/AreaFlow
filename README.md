...
- 本机 scoped API token、Web 登录和 authored workflow approval/rejection。
...
- Web 除读取外，只开放 authored workflow approval/rejection；其他写入仍关闭。
- run control 和 worker `run-once` 主要管理 AreaFlow 状态或 dry-run 任务，不等同于通用 AI engine 执行。
- users、teams、webhooks、secret resolve、remote workers 和第三方 plugin execution 尚未作为当前产品能力开放。
- API token 当前是本机 project/capability-scoped 认证，不代表 team/remote authorization 已开放。
...
该命令检查 Go 格式，运行后端测试与构建、Web/Desktop TypeScript/Vite 构建、文档治理和品牌素材校验。CI 还会在隔离 PostgreSQL 数据库中运行 Web browser smoke。
