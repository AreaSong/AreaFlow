...
[`auth-team-secret-boundary.md`](auth-team-secret-boundary.md)。在对应 R4 rung 通过前，schema row 只能作为
...
[`budget-quota-boundary.md`](budget-quota-boundary.md)。这些表是长期目标模型；在对应 B-rung 通过前，
...
[`integration-webhook-boundary.md`](integration-webhook-boundary.md)。这些表在对应 I-rung 通过前只能作为
...
要求见 [`command-approval-contract.md`](command-approval-contract.md)。本节只描述数据落点。
