...
[`security-permissions.md`](security-permissions.md)，目标是让 runner、worker、API、Web、Desktop
...
属于 R4，必须按 [`auth-team-secret-boundary.md`](auth-team-secret-boundary.md) 另行打开；remote worker
...
[`auth-team-secret-boundary.md`](auth-team-secret-boundary.md) 定义的 short-lived scoped binding；该
