...
- [`../architecture/workflow-engine-contract.md`](../architecture/workflow-engine-contract.md)：workflow engine 契约。
- [`../architecture/v0.1-import-mirror-contract.md`](../architecture/v0.1-import-mirror-contract.md)：v0.1
...
- [`../architecture/v0.2-shadow-doctor-contract.md`](../architecture/v0.2-shadow-doctor-contract.md)：v0.2
  Shadow Doctor、Drift Check、readiness、import-diff、verify-bundle 和 native doctor 授权边界。
- [`../architecture/v0.3-version-authoring-contract.md`](../architecture/v0.3-version-authoring-contract.md)：v0.3
...
- [`../architecture/v0.5-runner-preview-contract.md`](../architecture/v0.5-runner-preview-contract.md)：v0.5
  runner preview、dry-run run control、runner preview report 和 no-execution 边界。
- [`../architecture/v0.6-worker-beta-contract.md`](../architecture/v0.6-worker-beta-contract.md)：v0.6
  worker lifecycle、lease、dry-run run-once、scoped execution evidence 和 no-cutover 边界。
- [`../architecture/v0.7-web-dashboard-contract.md`](../architecture/v0.7-web-dashboard-contract.md)：v0.7
  Web Dashboard 的 `/api/v1`、SSE、read-only panels、write action gate 和 no-second-state 边界。
- [`../architecture/v0.8-multi-project-worker-pool-contract.md`](../architecture/v0.8-multi-project-worker-pool-contract.md)：v0.8
  worker pool summary、schedule preview、project isolation、readiness 和 no-scheduler 边界。
- [`../architecture/v0.9-desktop-shell-contract.md`](../architecture/v0.9-desktop-shell-contract.md)：v0.9
  local service status、dashboard launcher、desktop gates 和 no-second-state 边界。
- [`../architecture/v1.0-stable-platform-contract.md`](../architecture/v1.0-stable-platform-contract.md)：v1.0
  completion audit、release final gate、AreaMatrix dogfood completion、protected path proof 和 high-risk apply
  closed 边界。
- [`../architecture/data-model-v1.md`](../architecture/data-model-v1.md)：v1 数据模型。
- [`../architecture/command-approval-contract.md`](../architecture/command-approval-contract.md)：Command API、
  approval、permission、risk、audit 和 apply 的统一写入口合同。
- [`../architecture/high-risk-apply-ladder.md`](../architecture/high-risk-apply-ladder.md)：v1.x
  high-risk real apply 从 closed / preview 到 scoped production 的统一开闸阶梯。
- [`../architecture/artifact-backup-restore-contract.md`](../architecture/artifact-backup-restore-contract.md)：
...
- [`../architecture/integration-webhook-boundary.md`](../architecture/integration-webhook-boundary.md)：
  external integrations、webhooks、third-party callbacks 和多 API 接入边界。
- [`../architecture/budget-quota-boundary.md`](../architecture/budget-quota-boundary.md)：budget、quota、
...
- [`../architecture/worker-scheduling-contract.md`](../architecture/worker-scheduling-contract.md)：多项目 worker
  调度、lease、并发、recovery 和 preview / scheduler 边界。
- [`../architecture/auth-team-secret-boundary.md`](../architecture/auth-team-secret-boundary.md)：auth、team、
  API token、secret resolve 和 remote worker credential 的 R4 开闸边界。
- [`../architecture/plugin-marketplace-boundary.md`](../architecture/plugin-marketplace-boundary.md)：v1.0
  plugin / marketplace seed、manifest、conformance 和 v1.x plugin execution 边界。
- [`../migration/areamatrix-workflow-migration.md`](../migration/areamatrix-workflow-migration.md)：AreaMatrix 迁移路线。
...
| Release 模型 | v1.0 只做 release readiness、exception、final gate、evidence、package/distribution/publish/rollout preview；真实 package、tag、sign、push、upload、publish 放到 v1.x。完整合同见 [`release-final-gate-contract.md`](../architecture/release-final-gate-contract.md)。 |
| Completion audit | 100% 不能靠口头判断、绿色测试或 release final gate 单独证明；必须按 [`completion-audit-contract.md`](../architecture/completion-audit-contract.md) 聚合 phase/task、AreaMatrix dogfood、release、ops、isolation 和 protected path evidence。 |
| 运维 / 部署 / 可观测性 | v1.0 只做本机 install/migrate/start/status/doctor、metadata-only support bundle preview、本地日志脱敏和 telemetry local-only；远程运维控制、托管升级、破坏性 rollback 和完整 support export 放到 v1.x。完整合同见 [`operations-deployment-observability-boundary.md`](../architecture/operations-deployment-observability-boundary.md)。 |
...
[`../architecture/command-approval-contract.md`](../architecture/command-approval-contract.md)。
...
[`../architecture/execution-opening-strategy.md`](../architecture/execution-opening-strategy.md)；
...
[`../architecture/worker-scheduling-contract.md`](../architecture/worker-scheduling-contract.md)。
...
[`../architecture/release-final-gate-contract.md`](../architecture/release-final-gate-contract.md)。该合同是
...
[`../architecture/high-risk-apply-ladder.md`](../architecture/high-risk-apply-ladder.md)。本节只保留总顺序；
具体开闸证据以该合同为准。
...
9. **真实 secret resolve**：进入 R4。项目配置只引用 `secret_ref`，真实 secret 按
   [`auth-team-secret-boundary.md`](../architecture/auth-team-secret-boundary.md) 管理；worker 最多获得短期
...
14. **第三方 plugin execution**：必须先有 manifest、capability declaration、signature、
   ...
   [`plugin-marketplace-boundary.md`](../architecture/plugin-marketplace-boundary.md)。
15. **External integrations / webhooks**：按
   [`integration-webhook-boundary.md`](../architecture/integration-webhook-boundary.md) 逐级打开
...
16. **Team console**：按
   [`team-remote-control-boundary.md`](../architecture/team-remote-control-boundary.md) 逐级打开
...
17. **Object artifact store**：按
   [`object-artifact-retention-contract.md`](../architecture/object-artifact-retention-contract.md) 逐级打开
...
18. **Budget / quota enforcement**：按
   [`budget-quota-boundary.md`](../architecture/budget-quota-boundary.md) 逐级打开 metadata/readiness、
