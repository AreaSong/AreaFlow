...
后端 Worker 集合支持 worker key、status、worker type、capability 和 opaque cursor。Web registry 支持搜索、排序和客户端分页；Worker 详情按稳定 URL 加载最近 heartbeat 与 lease 历史。两类历史当前共享 history limit，尚无独立 cursor。
