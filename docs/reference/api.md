...
`AREAFLOW_AUTH_MODE=token` 时使用 `Authorization: Bearer <token>`。Token 绑定 project 和 capability scope；跨 project 访问返回 `404`。当前 token 模式只开放读取和 authored workflow 的 approval/rejection 写入，其他写 endpoint 保持关闭。
...
`/api/v1/health` 是进程存活探针。`/api/v1/ready` 检查 PostgreSQL 连接，数据库不可用或健康检查超时时返回 `503`。
...
Token 模式下 workflow approval 的 actor 由服务端 principal 决定，客户端 body 不能冒充 actor，并且必须提供 `Idempotency-Key` header。
